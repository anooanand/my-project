  ...
  isWebContainer ? {
    global: {
      fetch: async (url, options = {}) => {
        ...
        // Check if this is an auth API call that needs proxying
        if (url.toString().includes('/auth/v1/')) {
          const proxyUrl = url.toString().replace(supabaseUrl, '/.netlify/functions/supabase-auth');
          console.log(`Proxying Supabase auth call: ${url} -> ${proxyUrl}`);
          return fetch(proxyUrl, options);
        }
