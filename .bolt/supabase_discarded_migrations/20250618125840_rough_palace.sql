/*
  # Fix User Profiles and Improve Error Handling

  1. Changes
    - Fix user_profiles table structure
    - Add missing user_id field to user_profiles
...
CREATE POLICY IF NOT EXISTS "Users can view own profile by user_id" 
  ON user_profiles FOR SELECT 
  USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can update own profile by user_id" 
  ON user_profiles FOR UPDATE 
  USING (auth.uid() = user_id);

CREATE POLICY IF NOT EXISTS "Users can insert own profile by user_id" 
